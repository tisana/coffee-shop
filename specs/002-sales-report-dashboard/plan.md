...
**Testing**: Vitest for shared/domain/API aggregation and component tests; Supertest-style API contract tests; Playwright for the `#reports` staff dashboard flow  
...
**Post-Design Constitution Check**: PASS. The research, data model, contracts, and quickstart preserve the staff workflow, daily order identity, queue-state read-only boundary, purchased detail preservation, and story-by-story delivery path. No constitution violations require complexity tracking.
